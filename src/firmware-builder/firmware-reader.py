...
class MD5:
  ...
  def update(self, msg):
    ...
    for i in range(blocks):
      self.blockHash(self.buffer[i*64:(i+1)*64])
  ...
  def hexdigest(self):
    ...
    zeroPad //= 8
    self.buffer = self.buffer + b'\x00'*zeroPad + msgLen.to_bytes(8,byteorder='little')
    self.update(bytes())
...
def doFirmwareUpgrade(fwFilename, devType="00000000", path='/'):
  ...
  with open(fwFilename, 'rb') as fwbp:
    ...
    for n in range(0, numFiles):
      ...
      print("File [%s] size=[%d] offset=[%d]" % (fileName, fileSize, fileOffset))
...
def main(inputFilename, outputPath):
  doFirmwareUpgrade(inputFilename, "MRBWWIFI", outputPath)
  


if __name__ == '__main__':
  inputFilename = 'firmware.bin'
  try:
  ...
  sys.exit(main(inputFilename, outpath))
